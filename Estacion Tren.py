    ...
    def quitarVagon(self, pos):
        ...
        if pos < 0 or pos > self.carga -1:
            ...
            if pos == 0:
                ...
                self.capacidad -= self.tail.capacidad
                temp = self.tail
                self.tail = temp.prev
                temp.prev = None
                self.tail.next = None
    ...
    if tren == None:
        ...
        def armar2():
            ...
            for vagon in vagonesLibres:
                ...
                boton3 = Button(c_armar, image=botonFinal, command=lambda vagon=vagon: engancharFinal(vagon.id), bg="#313139", relief=FLAT)#Arreglar comando
...
def refresh ():
    ...
    for train in trains:
        if train.enEstacion and train.get_hora()[0] + 1 <= datetime.datetime.now().hour and train.get_hora()[0] >= datetime.datetime.now().hour - 1:#or train.get_hora()[0] == datetime.datetime.now().hour -1) and train.enEstacion:
...
def timer():
    while enEjecucion:
        ...
        for tren in trains_copy:
            if tren.enEstacion == True and (hora[0] > tren.hora[0] + 1 ): #Elimina los trenes que ya pasaron
                trains.remove(tren)
